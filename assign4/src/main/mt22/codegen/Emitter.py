...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is IntegerType:
            ...
            return "[" + self.getJVMType(ArrayType(inType.dimensions[1:], inType.typ) if len(inType.dimensions)>1 else inType.typ)
    ...
    *    @param typ the type of the constant
    ...
    def emitALOAD(self, in_, frame):
        ...
        elif type(in_) in [cgen.ClassType, StringType, ArrayType]:
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        elif type(inType) in [cgen.ClassType, StringType, ArrayType]:
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        if type(in_) in [ArrayType, StringType]:
            frame.pop()
    ...
    *   @return code Label<label>:
    ...
    def emitLABEL(self, label, frame):
        ...
        return self.jvm.emitLABEL(str(label))
    ...
    *   @param in the code to be printed out
    ...
    def emitAASTORE(self, in_, frame):
        frame.pop()
        frame.pop()
        frame.pop()
        if type(in_) in [IntegerType]:
    ...
    '''
    def emitNEWARRAY(self, in_, count, frame):
        buff = list()
        buff.append(self.emitPUSHICONST(count,frame))
        if type(in_) is IntegerType:
            buff.append(self.jvm.emitNEWARRAY('int'))
        elif type(in_) is FloatType:
            buff.append(self.jvm.emitNEWARRAY('float'))
        elif type(in_) is BooleanType:
            buff.append(self.jvm.emitNEWARRAY('boolean'))
        elif type(in_) is StringType:
            buff.append(self.jvm.emitANEWARRAY("java/lang/String"))
        elif type(in_) is ArrayType:
            buff.append(self.jvm.emitANEWARRAY(self.getJVMType(in_)))
        return  ''.join(buff)
    def emitMULTIANEWARRAY(self, type, dimen, frame): pass
